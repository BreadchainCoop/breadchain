...
interface IDynamicNFTMultiplier is INFTMultiplier {
    ...
    /// @return factor The multiplying factor for the _user as a percentage where 10000 = 100%
    function userToFactor(address _user) external view returns (uint256 factor);

    /// @notice Get the validity period for a _user's factor
    /// @param _user The address of the _user
    /// @return validityTimestamp The timestamp until which the _user's factor is valid
    function _userToValidity(address _user) external view returns (uint256 validityTimestamp);
}
